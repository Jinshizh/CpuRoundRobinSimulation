...
//        total_customer_stay_time += current_event->occur_time - windows[current_event->event_type].getArrivaltime();
        total_customer_stay_time += TIME_SEGMENT;   //顾客逗留时间 += 时间片时间

        //当前服务的进程 duration 如果大于时间片时间，则将 duration 减去时间片的时间再重新插入到等待队列中，
        if(windows[current_event->event_type].getDuration() > TIME_SEGMENT) {
            // 将新的事件重新插入到事件表中
            Event re_event(customer_list.getLastCustomer()->arrival_time + TIME_SEGMENT); //保证了时间顺序，这样才能插到事件队列的队尾
            event_list.enqueue(re_event);

            // 将当前顾客重新插入到等待队列中
            Customer re_customer(customer_list.getLastCustomer()->arrival_time + TIME_SEGMENT,
                                 windows[current_event->event_type].getDuration() - TIME_SEGMENT);
            customer_list.enqueue(re_customer);
        }
